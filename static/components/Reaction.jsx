                ...
                {isShown && (
                    <div className="reactions">
                        ...
                        {showLike && !props.usersWhoLiked.includes(props.loggedInUser) ?
                            <button className="reactions" onClick={() => {setIsShown(false), addReaction('Like'), setShowLike(false), setShowUnlike(true)}}>👍</button>
                        : <div className="reactions"></div>
                        }
                        
                        {props.usersWhoLiked.includes(props.loggedInUser) ?
                            <button className="reactions" onClick={() => {setIsShown(false), undoReaction('Like'), setShowLike(true)}}>❌👍</button>
                        : <div className="reactions"></div>
                        }
                      
                        {showLove && !props.usersWhoLoved.includes(props.loggedInUser) ?
                            <button className="reactions" onClick={() => {setIsShown(false), addReaction('Love'), setShowLove(false), setShowUnlove(true)}}>💗</button>
                        : <div className="reactions"></div>
                        }
                        
                        {props.usersWhoLoved.includes(props.loggedInUser) ?
                            <button className="reactions" onClick={() => {setIsShown(false), undoReaction('Love'), setShowLove(true)}}>❌💗</button>
                        : <div className="reactions"></div>
                        }
                        
                        {showHaha && !props.usersWhoHaha.includes(props.loggedInUser) ?
                            <button className="reactions" onClick={() => {setIsShown(false), addReaction('Ha ha!'), setShowHaha(false), setShowUnhaha(true)}}>😂</button>
                        : <div className="reactions"></div>
                        }
                        
                        {props.usersWhoHaha.includes(props.loggedInUser) ?
                            <button className="reactions" onClick={() => {setIsShown(false), undoReaction('Ha ha!'), setShowHaha(true)}}>❌😂</button>
                        : <div className="reactions"></div>
                        }

                        {showHug && !props.usersWhoHugged.includes(props.loggedInUser) ?
                            <button className="reactions" onClick={() => {setIsShown(false), addReaction('Hug'), setShowHug(false), setShowUnhug(true)}}>🤗</button> 
                        : <div className="reactions"></div>
                        }
                        
                        {props.usersWhoHugged.includes(props.loggedInUser) ?
                            <button className="reactions" onClick={() => {setIsShown(false), undoReaction('Hug'), setShowHug(true)}}>❌🤗</button>
                        : <div className="reactions"></div>
                        }
